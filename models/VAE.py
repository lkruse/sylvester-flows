...
class HouseholderVAE(VAE):
    ...
    def __init__(self, args):
        super(HouseholderVAE, self).__init__(args)

        # Initialize log-det-jacobian to zero
        self.log_det_j = 0.

        # Flow parameters
        flow = flows.Householder
        ...
        self.amor_k = nn.Linear(self.q_z_nn_output_dim, self.num_flows)
    ...
    def encode(self, x):
        ...
        w = self.amor_w(h).view(batch_size, self.num_flows, self.z_size, 1)
        b = self.amor_b(h).view(batch_size, self.num_flows, 1, 1)
        k = self.amor_k(h).view(batch_size, self.num_flows, 1, 1)

        return mean_z, var_z, w, b, k
    ...
    def forward(self, x):
        ...
        z_mu, z_var, w, b, k = self.encode(x)
        ...
        for i in range(self.num_flows):
            flow_k = getattr(self, 'flow_' + str(i))
            z_k, log_det_jacobian = flow_k(z[i], w[:, i, :, :], b[:, i, :, :], k[:, i, :, :])
            z.append(z_k)
